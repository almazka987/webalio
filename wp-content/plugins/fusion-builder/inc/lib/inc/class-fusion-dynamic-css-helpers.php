...
class Fusion_Dynamic_CSS_Helpers {
	...
	public function dynamic_css_cached() {
		...
		if ( 'db' === $settings->get( 'css_cache_method' ) ) {
			$cache = true;
		}
		// If WP_DEBUG set to true then do not cache.
		...
		$mode = $dynamic_css_obj->mode;
		$cache_time = ( 'inline' === $mode ) ? DAY_IN_SECONDS : HOUR_IN_SECONDS;
