	...
	function fusion_builder_render_post_metadata( $layout, $settings = array() ) {
		...
		if ( ( $settings['post_meta'] && 'no' !== $post_meta ) || ( ! $settings['post_meta'] && 'yes' === $post_meta ) ) {
			...
			if ( $settings['post_meta_author'] ) {
				...
				if ( ! $fusion_settings->get( 'disable_date_rich_snippet_pages' ) ) {
					$metadata .= sprintf( esc_html__( 'By %s', 'fusion-library-textdomain' ), '<span>' . $author_post_link . '</span>' );
				} else {
					$metadata .= sprintf( esc_html__( 'By %s', 'fusion-library-textdomain' ), '<span class="vcard"><span class="fn">' . $author_post_link . '</span></span>' );
				}
			...
			if ( $settings['post_meta_cats'] ) {
				...
				if ( $categories ) {
					$metadata .= ( $settings['post_meta_tags'] ) ? sprintf( esc_html__( 'Categories: %s', 'fusion-library-textdomain' ), $categories ) : $categories;
					$metadata .= '<span class="fusion-inline-sep">|</span>';
				}
			}
			...
			if ( $settings['post_meta_tags'] ) {
				...
				if ( $tags ) {
					$metadata .= '<span class="meta-tags">' . sprintf( esc_html__( 'Tags: %s', 'fusion-library-textdomain' ), $tags ) . '</span><span class="fusion-inline-sep">|</span>';
				}
			}
			...
			if ( $settings['post_meta_comments'] && 'grid_timeline' !== $layout ) {
				ob_start();
				comments_popup_link( esc_html__( '0 Comments', 'fusion-library-textdomain' ), esc_html__( '1 Comment', 'fusion-library-textdomain' ), esc_html__( '% Comments', 'fusion-library-textdomain' ) );
