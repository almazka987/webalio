	...
	class Fusion_Slider {
		...
		public function init() {
			...
			if ( $fusion_settings->get( 'status_fusion_slider' ) ) {
				...
				Fusion_Dynamic_JS::enqueue_script(
					'avada-fusion-slider',
					wp_normalize_path( FUSION_CORE_PATH . '/js/min/avada-fusion-slider.js' ),
					$dependencies,
					'1',
					true
				);
		...
		function admin_init() {
			...
			if ( ( isset( $_GET['taxonomy'] ) && 'slide-page' === $_GET['taxonomy'] ) || ( isset( $_GET['post_type'] ) && 'slide' === $_GET['post_type'] ) || 'slide' === $post_type ) {
				wp_enqueue_script( 'fusion-slider', esc_url_raw( FUSION_CORE_URL . '/js/fusion-slider.js' ), false, '1.0', true );
			}
		...
		public function slider_add_new_meta_fields() {
			...
			include wp_normalize_path( FUSION_CORE_PATH . '/fusion-slider/templates/add-new-meta-fields.php' );
		...
		public function slider_edit_meta_fields( $term ) {
			...
			include wp_normalize_path( FUSION_CORE_PATH . '/fusion-slider/templates/edit-meta-fields.php' );
		...
		public function fs_export_import_settings() {
			...
			include wp_normalize_path( FUSION_CORE_PATH . '/fusion-slider/templates/export-import-settings.php' );
		}
		 ...
		 */
		public function import_sliders( $zip_file = '' ) {
			if ( isset( $zip_file ) && '' !== $zip_file ) {
				$upload_dir = wp_upload_dir();
				$base_dir   = trailingslashit( $upload_dir['basedir'] );
				$fs_dir     = $base_dir . 'fusion_slider_exports/';

				$this->filesystem()->delete( $fs_dir . 'sliders.xml' );
				$this->filesystem()->delete( $fs_dir . 'settings.json' );
				...
				if ( ! class_exists( 'WP_Import' ) ) {
					require_once wp_normalize_path( FUSION_CORE_PATH . '/libs/wordpress-importer.php' );
				}
		...
		public function admin_clone_slider_button_edit_form( $term ) {
			...
			if ( isset( $_GET['taxonomy'] ) && 'slide-page' === $_GET['taxonomy'] && current_user_can( 'manage_options' ) ) {
				...
				$url = add_query_arg( $args, admin_url( 'edit-tags.php' ) );
				include wp_normalize_path( FUSION_CORE_PATH . '/fusion-slider/templates/clone-button-edit-form.php' );
			}
		}
		...
		public function admin_clone_slide_button_after_title( $post ) {
			if ( isset( $_GET['post'] ) && current_user_can( 'manage_options' ) && 'slide' === $post->post_type ) {
				include wp_normalize_path( FUSION_CORE_PATH . '/fusion-slider/templates/clone-button-after-title.php' );
			}
		}
		...
		public function save_as_new_slide() {
			...
			if ( isset( $_REQUEST['_fusion_slide_clone_nonce'] ) && check_admin_referer( 'clone_slide', '_fusion_slide_clone_nonce' ) && current_user_can( 'manage_options' ) ) {
				...
				if ( isset( $post ) && $post ) {
					...
					wp_die( sprintf( esc_attr__( 'Cloninig failed. Post not found. ID: %s', 'fusion-core' ), wp_kses_post( htmlspecialchars( $id ) ) ) );
				}
			}
		}
		...
		public static function render_fusion_slider( $term ) {
			...
			if ( $fusion_settings->get( 'status_fusion_slider' ) ) {
				...
				if ( $query->have_posts() ) {
					include wp_normalize_path( FUSION_CORE_PATH . '/fusion-slider/templates/slider.php' );
				}
